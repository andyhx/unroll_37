...
class UnrollingLayer : public Layer<Dtype> {
 public:
  explicit UnrollingLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  bool LoadPca();
  ...
  virtual inline const char* type() const { return "Unrolling"; }
/*
...
*/
 protected:
  ...
  cv::Mat unrolling(eos::render::Mesh mesh,cv::Mat affine_matrix,cv::Mat image,int num);
  void unrolling_backward_cpu(Dtype dV, const Dtype* U, const Dtype px,
