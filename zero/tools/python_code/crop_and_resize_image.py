#just integrate two .py files
import cv2
from  cv2 import cv
import os
out_dir="/home/brl/data/300w/crop_image_vn"
...
for file_dir in all_file_dir:
    ...
    for file in all_file:
        if file.endswith('box.txt'):
            ...
            img=cv2.cvtColor(crop_img,cv2.cv.CV_BGR2GRAY)
            img=cv2.resize(img,(img_resize,img_resize)) 
            cv2.imwrite(out_dir+os.path.sep+img_name,img)
